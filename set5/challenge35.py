...
################################################################################
    ...
    #print hex(s_a)
    if s_a > 0xf:  # it's gonna use sha1("\x01")[:16] as the key
    ...
    msg_m = cbc_decrypt(aes_ecb_m, m_data3[:-16], m_data3[-16:]) #M knows the message (DONE)
...
################################################################################
    ...
    msg_a = "Super Secret Stuff123"
    #print hex(s_a)
    if s_a > 0xf:  # it's gonna use sha1("\x00")[:16] as the key
        key_a = sha1(hex(s_a)[2:].replace('L','').decode('hex'))[:16]
    else:
        key_a = sha1(("0"+hex(s_a)[2:].replace('L','')).decode('hex'))[:16]
    aes_ecb_a = AES.new(key_a, AES.MODE_ECB)
...
################################################################################
    ...
    key_m1 = sha1("\x01")[:16]
    key_m2 = sha1(hex(p-1)[2:].replace('L','').decode('hex'))[:16]
