...
# https://www.cryptopals.com/sets/4/challenges/32
...
def find_valid_hmac(name):
    ...
    while len(x) < 20:
        ...
        for c in charset:
            a = []
            for i in range(40):
                r = requests.get(url+(x+c).encode("hex"))
                ...
                a.append(t)
                a.sort()
                trimmed = a[:-4] #removing possible outliers
            temp.append(sum(trimmed)/len(trimmed))
            print (x+c).encode("hex")
            print "8324fcc04874fc7e7a504524ee0b39bad319eea4"
        x += chr(temp.index(max(temp)))
        print x.encode("hex")
